...
def train_model(dataset, epochs=run_condition.iloc[0,3], lr=0.001, log_steps=1):
    ...
    for epoch in range(epochs):
        ...
        for i, alloy_generator in enumerate(dataset):
            ...
            reny = alloy_generator.data_sampling(dataset)#single_random_sampling(3)
            #print(reny)
            x = reny.iloc[:,:18].values
            y = reny.iloc[:,18:].values
...
def train_maml(model, epochs, dataset, lr_inner=0.001, log_steps=1):
    ...
    for epoch in range(epochs):
        ...
        # Step 3 and 4
        for i, alloy_generator in enumerate(dataset):
            reny1, reny2 = alloy_generator.data_sampling(dataset)
        
            train_x = reny1.iloc[:,:18].values
            train_y = reny1.iloc[:,18:].values
                    
            test_x = reny2.iloc[:,:18].values
            test_y = reny2.iloc[:,18:].values
...
def train_transfer(model, epochs, dataset, log_steps=1, lr=0.001, plot=True):
    ...
    reny = alloy_generator.data_sampling(test_ds)
     
    
    optimizer = keras.optimizers.SGD(learning_rate=lr) 

    x = list(reny.iloc[:,:18].values)
    y = list(reny.iloc[:,18:].values)
    x = np.array(x)
    y = np.array(y)
    x[:,None]
    y[:,None]
    ...
    for epoch in range(epochs):
        ...
        for i, alloy_generator in enumerate(dataset):
            ...
            reny2 = alloy_generator.data_sampling(dataset)
            #print(reny)
            x = reny2.iloc[:,:18].values
            y = reny2.iloc[:,18:].values
